...
int main(int Arguments, char* Argument[])
{
	...
	int CardNr;
	const char CardOrder[] = "AKQT98765432J";
	...
	tClass ClassifyHand(char* Card, int *Value)
	{
		...
		// Special treatment for Jacks
		int HighOccur=0, JackOccur=0;
		int HighIndex=0, JackIndex=-1;
		for (Index=0; Index<5; Index++)
		{
			if (!Occurs[Index])  break;
			if (ValueList[Index] == 13)
			{
				JackOccur = Occurs[Index];
				JackIndex = Index;
			}
			else if (Occurs[Index] > HighOccur)
			{
				HighOccur = Occurs[Index];
				HighIndex = Index;
			}
		} /* for (Index) */
		// Add the Jack occurrences to the highest other occurrences
		if (JackOccur)
		{
			Occurs[HighIndex] += JackOccur;
			// Shift all further Occurs down towards the JackIndex
			for (Index=JackIndex; Index<4; Index++)
			{
				Occurs[Index] = Occurs[Index+1];
				if (!Occurs[Index+1])  break;
			}
		}
		// Special case : all-Jacks
		if (JackOccur == 5)  return eFive;
