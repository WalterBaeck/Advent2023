...
int main(int Arguments, char* Argument[])
{
	...
	int CardNr, MatchNr, NrOfMatches, DoingWins;
	int NrOfCards[200];
	for (CardNr=1; CardNr<200; CardNr++)  NrOfCards[CardNr]=1;
    ...
    if (1 != sscanf(InputPtr, "%d", &CardNr))
		...
		// Doublecheck
		if (CardNr != InputLineNr)
		{
			fprintf(stderr, "CardNr %d on InputLine #%d ?\n", CardNr, InputLineNr);
			exit(4);
		}
		...
		for (HaveNr=0; HaveNr<NrOfHaves; HaveNr++)
			for (WinNr=0; WinNr<NrOfWins; WinNr++)
				if (Have[HaveNr] == Win[WinNr])
					NrOfMatches++;
		// Scoring
		for (MatchNr=0; MatchNr<NrOfMatches; MatchNr++)
			NrOfCards[CardNr+1+MatchNr] += NrOfCards[CardNr];
		Sum += NrOfCards[CardNr];
	} /* while (fgets) */
